# polymarketarb
polymarket arb 1xbet。polymarket 对1xbet 套利。
整个代码仓库后续会开源。目前因为是cursor编写的，作者也是第一次写插件。项目需要整理后，再开源。
这是一个chrome插件。
<img width="1906" height="1039" alt="image" src="https://github.com/user-attachments/assets/f33760b7-1d9e-4e75-bf46-98e757eda79c" />
## 插件下载
[下载 PolymarketArb v3.0.2](https://github.com/nbaertuo/polymarketarb/releases/tag/3.0.2)
下载后，直接解压。

## 插件安装
打开chrome的插件配置页面，勾选【开发者选项】，选择【加载已解压的插件文件】


## 插件配置
所有的apikey都存放在你的chrome本地存储。
<img width="2302" height="1192" alt="image" src="https://github.com/user-attachments/assets/5c091060-94d4-4b85-a98a-b63b227e8b44" />
[如何获取apikey](https://github.com/nbaertuo/polymarketarb/blob/main/Polymarket%E9%85%8D%E7%BD%AE.md)

## 电报群
[电报群](https://t.me/+aIVJFXCgNXg0ZGRh)


